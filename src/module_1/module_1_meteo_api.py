...
def request_API(urlAPI : str,**kwargs : dict) -> requests.models.Response:
    """
    Funcion que realiza una consulta a cualquier API, devolviendo la respuesta de la misma.
...
def get_data_meteo_api(ciudad: str,variables_temp: list) -> requests.models.Response:
    """
    Funcion que realiza una consulta a la API de Open Meteo, según los argumentos introducidos.
    
    Para hacer la consulta a la API utiliza la función request_API, creada anteriormente.
    
    Args:
        ciudad (str): Ciudad de la que se quiere obtener los datos.
        variables_temp (list): Lista de variables de temperatura que se quieren consultar.
    Returns:
        r (requests.models.Response): Respuesta de la API.
...
def data_API_to_df(data: requests.models.Response) -> pd.DataFrame:
    ...
    Coge el texto de la respuesta, lo convierte a un diccionario y posteriormente en un DataFrame.
...
def mensualizar_datos(df: pd.DataFrame,funciones:dict) -> pd.DataFrame:
    ...
    Agrupa los datos por mes y hace el cálculo correspondiente para cada variable a partir del diccionario añadido.
...
def graficoVariable(dataframes: list, variable: str, tipo: str, labels: list) -> None:
    """
    Función que realiza una grafico sobre la variable introducida desde diferentes dataframes en un solo gráfico.
    Con el lineplot se puede ver la evolución de la variable en el tiempo. Con el histplot se puede ver la distribución de la variable.
